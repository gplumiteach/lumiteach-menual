...
const manualAssetVersion = '20260714-ai-make-white-icon';
...
function withAssetVersion(assetPath) {
  const separator = String(assetPath || '').includes('?') ? '&' : '?';
  return `${assetPath}${separator}v=${manualAssetVersion}`;
}
...
function getManualImageCandidates(imagePath) {
  ...
  return candidates
    .filter((candidate, index) => candidate && candidates.indexOf(candidate) === index)
    .map((candidate) => withAssetVersion(candidate));
}
...
function getSafeLocalizedCopy(value) {
  const normalizedValue = normalizeTranslationTerms(value || '');
  if (!normalizedValue) return '';
  if (currentLanguage !== defaultLanguage && hasKoreanText(normalizedValue)) return '';

  return normalizedValue;
}

function getExplicitHubDescription(key) {
  return getSafeLocalizedCopy(activeTranslations().hubDescriptions?.[key]);
}

function getArticleSummaryDescription(article) {
  const localized = localizeArticle(article);
  return getSafeLocalizedCopy(localized.desc) || getSafeLocalizedCopy(localized.note);
}

function cleanBodySummaryText(value) {
  const summaryBlocks = String(value || '')
    .replace(/\[\[image:[^\]]+\]\]/g, '\n\n')
    .replace(/\[\[callout:[^\]]+\]\]/g, '\n\n')
    .replace(/^#{1,6}\s+/gm, '')
    .replace(/^>\s?/gm, '')
  ...
  return (summaryBlocks.find((block) => block.trim()) || '')
    .replace(/\s+/g, ' ')
    .trim();
}

function getArticleBodySummaryDescription(article) {
  ...
  for (let index = 0; index < bodyCount; index += 1) {
    const body = tArticleBody(article.key, index, defaultBodies[index] || '');
    const summary = getSafeLocalizedCopy(cleanBodySummaryText(body));
...
function getHubDescription(article) {
  const explicitDescription = getExplicitHubDescription(article.key);
  if (explicitDescription) return explicitDescription;

  const articleDescription = getArticleSummaryDescription(article);
  if (currentLanguage !== defaultLanguage && articleDescription) return articleDescription;

  const bodyDescription = getArticleBodySummaryDescription(article);
  if (currentLanguage !== defaultLanguage && bodyDescription) return bodyDescription;

  return hubDescriptions[article.key] || articleDescription || bodyDescription || '';
}
...
function renderHubIcon(visual) {
  if (visual.iconPath) {
    return `<img class="hub-icon-img" src="${escapeHtml(withAssetVersion(visual.iconPath))}" alt="" loading="lazy" />`;
  }
...
function getHubDescriptionForSearch(article, localizedArticle) {
  return getHubDescription(article) || localizedArticle.desc || '';
}
